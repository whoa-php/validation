 ...
 */
final class StringArrayToStringArray extends ExecuteRule
{
    /**
     * @inheritDoc
     */
    public static function execute($value, ContextInterface $context, $extras = null): array
    {
        ...
        if (is_iterable($value) === true) {
            foreach ($value as $key => $mightBeString) {
                if (is_string($mightBeString) === true || is_numeric($mightBeString) === true) {
                    $result[$key] = (string)$mightBeString;
                } else {
                    $reply = StringArrayToStringArray::createErrorReply(
                        $context,
                        $mightBeString,
        ...
        return $reply !== null ? $reply : StringArrayToStringArray::createSuccessReply($result);
    }
}
